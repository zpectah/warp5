...
class Deliveries {
	...
	public function create ($conn, $requestData, $languages) {
		...
		// prepare
		$query = ('INSERT INTO deliveries (name, type, item_price, item_weight_limit, img_main, active, deleted) VALUES (?,?,?,?,?,?,?)');
		$types = 'ssiisii';
		$args = [
			$requestData['name'],
			$requestData['type'],
	...
	public function update ($conn, $requestData, $languages) {
		...
		// prepare
		$query = ('UPDATE deliveries SET name = ?, type = ?, item_price = ?, item_weight_limit = ?, img_main = ?, active = ? WHERE id = ?');
		$types = 'ssiisii';
		$args = [
			$requestData['name'],
			$requestData['type'],
	...
	public function toggle ($conn, $requestData) {
		...
		function toggleRow ($conn, $id) {
			// prepare
			$query = ('UPDATE deliveries SET active = IF(active=1, 0, 1) WHERE id = ?');
			$types = 'i';
			$args = [ $id ];
	...
	public function delete ($conn, $requestData) {
		...
		function deleteRow ($conn, $id) {
			// prepare
			$query = ('UPDATE deliveries SET deleted = 1 WHERE id = ?');
			$types = 'i';
			$args = [ $id ];
