...
class Producers {
	...
	public function create ($conn, $requestData) {
		...
		// prepare
		$query = ('INSERT INTO producers (name, type, img_main, active, deleted) VALUES (?,?,?,?,?)');
		$types = 'sssii';
		$args = [
			$requestData['name'],
			$requestData['type'],
	...
	public function update ($conn, $requestData) {
		...
		// prepare
		$query = ('UPDATE producers SET name = ?, type = ?, img_main = ?, active = ? WHERE id = ?');
		$types = 'sssii';
		$args = [
			$requestData['name'],
			$requestData['type'],
	...
	public function toggle ($conn, $requestData) {
		...
		function toggleRow ($conn, $id) {
			// prepare
			$query = ('UPDATE producers SET active = IF(active=1, 0, 1) WHERE id = ?');
			$types = 'i';
			$args = [ $id ];
	...
	public function delete ($conn, $requestData) {
		...
		function deleteRow ($conn, $id) {
			// prepare
			$query = ('UPDATE producers SET deleted = 1 WHERE id = ?');
			$types = 'i';
			$args = [ $id ];
