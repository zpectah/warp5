...
import { useMenu, useProfile } from '../../hooks/App';
...
const MenuPage = () => {
	...
	const modelRoute = ROUTES.app.menu;
	const columnsLayout = {
		name: true,
		type: true,
		active: true,
	...
	const onToggle = (ids: number[]) => {
		...
		toggleMenu(ids).then((response) => {
			// console.log('toggleMenu', response);
	...
	useEffect(() => {
		if (params.id && Menu) {
			setDetailOpen(true);
			setDetailData(getDetailData(params.id, Menu));
		}
	}, [params.id, Menu]);
