...
const PostsPage = () => {
	...
	const modelRoute = ROUTES.app.posts;
	const columnsLayout = {
		title_lang: true,
		type: true,
		active: true,
	...
	const onToggle = (ids: number[]) => {
		...
		togglePosts(ids).then((response) => {
			// console.log('togglePosts', response);
	...
	const onDataSubmit = (data: any) => {
		...
		if (data.id == 'new') {
			createPosts(master).then((response) => {
				// console.log('createPosts', response);
			...
			updatePosts(master).then((response) => {
				// console.log('updatePosts', response);
	...
	useEffect(() => {
		if (params.id && Posts) {
			setDetailOpen(true);
			setDetailData(getDetailData(params.id, Posts));
		}
	}, [params.id, Posts]);
