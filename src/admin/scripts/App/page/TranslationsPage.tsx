...
import { useTranslations, useProfile } from '../../hooks/App';
...
const TranslationsPage = () => {
	...
	const modelRoute = ROUTES.app.translations;
	const columnsLayout = {
		t_value: true,
		active: true,
	};
	...
	const onToggle = (ids: number[]) => {
		...
		toggleTranslations(ids).then((response) => {
			// console.log('toggleTranslations', response);
	...
	const onDeleteConfirm = (ids: number[]) => {
		...
		deleteTranslations(master).then((response) => {
			// console.log('deleteTranslations', response);
	...
	const onDataSubmit = (data: any) => {
		...
		if (data.id == 'new') {
			createTranslations(master).then((response) => {
				// console.log('createTranslations', response);
			...
			updateTranslations(master).then((response) => {
				// console.log('updateTranslations', response);
	...
	useEffect(() => {
		if (params.id && Translations) {
			setDetailOpen(true);
			setDetailData(getDetailData(params.id, Translations));
		}
	}, [params.id, Translations]);

	// Trigger loading status
	useEffect(() => {
		setLoading(isTranslationsLoading);
	}, [isTranslationsLoading]);
