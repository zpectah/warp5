...
import { DistributorsItemProps } from '../../../types/Market';
...
interface DistributorsFormProps {
	detailData: DistributorsItemProps;
	onDelete: (ids: any[]) => void;
	onSubmit: (data: DistributorsItemProps) => void;
	onCancel: () => void;
	allowDelete: boolean;
	processing?: boolean;
	loading?: boolean;
...
const DistributorsForm = ({
	...
	const checkDupes = (name: string) =>
		setDuplicates(checkDuplicates(Distributors, name, detailData.id));

	return (
		<>
			<DialogTitle>
				{detailData.id == 'new' ? t('btn_new.Distributors') : detailData.name}
			</DialogTitle>
			<DialogContent dividers>
				<Form.Base name="DistributorsForm">
					<div>
						<input
							type="hidden"
							name="id"
					...
					<Section withBorder>
						...
						<Form.RowController
							...
							defaultValue={
								detailData.type ||
								config.OPTIONS.model.Distributors.type_default
							}
						>
							{(row) => (
								<Form.Select
								...
								>
									{config.OPTIONS.model.Distributors.type_list.map((opt) => (
