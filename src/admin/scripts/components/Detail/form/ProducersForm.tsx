...
import { ProducersItemProps } from '../../../types/Market';
...
interface ProducersFormProps {
	...
	onSubmit: (data: ProducersItemProps) => void;
	onCancel: () => void;
	allowDelete: boolean;
	processing?: boolean;
	loading?: boolean;
...
const ProducersForm = ({
	...
	const checkDupes = (name: string) =>
		setDuplicates(checkDuplicates(Producers, name, detailData.id));

	return (
		<>
			<DialogTitle>
				{detailData.id == 'new' ? t('btn_new.Producers') : detailData.name}
			</DialogTitle>
			<DialogContent dividers>
				<Form.Base name="ProducersForm">
					<div>
						<input
							type="hidden"
							name="id"
					...
					<Section withBorder>
						...
						>
							{(row) => (
								...
								>
									{config.OPTIONS.model.Producers.type_list.map((opt) => (
