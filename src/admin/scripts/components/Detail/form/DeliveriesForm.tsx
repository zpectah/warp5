...
import { DeliveriesItemProps } from '../../../types/Market';
...
interface DeliveriesFormProps {
	...
	onSubmit: (data: DeliveriesItemProps) => void;
	onCancel: () => void;
	allowDelete: boolean;
	processing?: boolean;
	loading?: boolean;
...
const DeliveriesForm = ({
	...
	const checkDupes = (name: string) =>
		setDuplicates(checkDuplicates(Deliveries, name, detailData.id));
					...
					<Section withBorder>
						...
						>
							{(row) => (
								...
								>
									{config.OPTIONS.model.Deliveries.type_list.map((opt) => (
