...
const ProducersPage = () => {
	...
	// Static variables
	const modelName = 'Producers';
	const modelRoute = ROUTES.market.producers;
	const columnsLayout = {
		name: true,
		active: true,
	};
	...
	const onToggle = (ids: number[]) => {
		...
		toggleProducers(ids).then((response) => {
			// console.log('toggleProducers', response);
	...
	const onDeleteConfirm = (ids: number[]) => {
		...
		deleteProducers(master).then((response) => {
			// console.log('deleteProducers', response);
	...
	const onDataSubmit = (data: any) => {
		...
		if (data.id == 'new') {
			createProducers(master).then((response) => {
				// console.log('createProducers', response);
	...
	useEffect(() => {
		if (params.id && Producers) {
			setDetailOpen(true);
			setDetailData(getDetailData(params.id, Producers));
		}
	}, [params.id, Producers]);

	// Trigger loading status
	useEffect(() => {
		setLoading(isProducersLoading);
	}, [isProducersLoading]);
