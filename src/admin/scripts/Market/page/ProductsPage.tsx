...
const ProductsPage = () => {
	...
	const modelRoute = ROUTES.market.products;
	const columnsLayout = {
		title_lang: true,
		type: true,
		active: true,
	...
	const onToggle = (ids: number[]) => {
		...
		toggleProducts(ids).then((response) => {
			// console.log('toggleProducts', response);
	...
	const onDeleteConfirm = (ids: number[]) => {
		...
		deleteProducts(master).then((response) => {
			// console.log('deleteProducts', response);
	...
	useEffect(() => {
		if (params.id && Products) {
			setDetailOpen(true);
			setDetailData(getDetailData(params.id, Products));
		}
	}, [params.id, Products]);
