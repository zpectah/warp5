...
const DistributorsPage = () => {
	...
	const modelRoute = ROUTES.market.distributors;
	const columnsLayout = {
		name: true,
		active: true,
	};
	...
	const onToggle = (ids: number[]) => {
		...
		toggleDistributors(ids).then((response) => {
			// console.log('toggleDistributors', response);
	...
	const onDeleteConfirm = (ids: number[]) => {
		...
		deleteDistributors(master).then((response) => {
			// console.log('deleteDistributors', response);
	...
	const onDataSubmit = (data: any) => {
		...
		if (data.id == 'new') {
			createDistributors(master).then((response) => {
				// console.log('createDistributors', response);
			...
			updateDistributors(master).then((response) => {
				// console.log('updateDistributors', response);
	...
	useEffect(() => {
		if (params.id && Distributors) {
			setDetailOpen(true);
			setDetailData(getDetailData(params.id, Distributors));
		}
